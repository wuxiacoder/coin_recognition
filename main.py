...
image = cv2.imread("input/3sample.jpg")  # read in image
...
def calc_histogram(img):
    m2 = np.zeros(img.shape[:2], dtype="uint8")  # create mask
    (w, h) = (int(img.shape[1] / 2), int(img.shape[0] / 2))
    cv2.circle(m2, (w, h), 60, 255, -1)  # draw circle
    # calcHist expects a list of images, color channels, mask, bins, ranges
    h = cv2.calcHist([img], [0, 1, 2], m2, [8, 8, 8], [0, 256, 0, 256, 0, 256])
...
Material = Enum(('Ten', 'Five', 'One', 'TwenC', 'TenC', 'FiveC'))
...
sample_images_tencents = glob.glob("sample_images/tencents/*.png")
sample_images_twentyfivecents = glob.glob("sample_images/twentyfivecents/*.png")
...
for i in sample_images_twentyfivecents:
    X.append(calc_hist_from_file(i))
    y.append(Material.TwenC)
for i in sample_images_tencents:
    X.append(calc_hist_from_file(i))
    y.append(Material.TenC)
for i in sample_images_fivecents:
    X.append(calc_hist_from_file(i))
    y.append(Material.FiveC)
...
circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, dp=2.2, minDist=100, param1=100, param2=100, minRadius=10,
                           maxRadius=80)


def predict_material(roi2):
    hist = calc_histogram(roi2)  # calculate feature vector for region of interest
...
if circles is not None:
    ...
    for (x, y, d) in circles:  # loop over coordinates and radii of the circles
        ...
        material = predict_material(roi)  # predict material type
        ...
        cv2.putText(output, material, (x - 40, y), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), thickness=2,
...
if materials[i] == "Ten":
    ...
    diameter = [x / biggest * 18.75 for x in diameter]
    scaledTo = "Scaled to 1 Peso"
elif materials[i] == "TwenC":
    diameter = [x / biggest * 17.75 for x in diameter]
    scaledTo = "Scaled to 25 cents"
elif materials[i] == "TenC":
    diameter = [x / biggest * 16.25 for x in diameter]
    scaledTo = "Scaled to 10 cents"
elif materials[i] == "FiveC":
...
while i < len(diameter):  # loop over diameters
    ...
    if m == "Ten":  # if material is ten peso
        t = "10"
    elif m == "Five":  # if material is five peso
        t = "5"
    elif m == "One":  # if material is one peso
        t = "1"
    elif m == "TwenC":  # if material is one peso
        t = "25c"
    elif m == "TenC":  # if material is one peso
        t = "10c"
    elif m == "FiveC":  # if material is five cents
        t = "5c"
    else:  # if material is unknown
        t = "Unknown"

    # write result on output image
    cv2.putText(output, t, (x - 40, y + 22), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), thickness=2,
                lineType=cv2.LINE_AA)
    i += 1
...
cv2.putText(output, "Coins detected: {}, {:2}".format(count, total / 100), (5, output.shape[0] - 24),
            cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), lineType=cv2.LINE_AA)
cv2.putText(output, "Classifier mean accuracy: {}%".format(score), (5, output.shape[0] - 8), cv2.FONT_HERSHEY_PLAIN,
